...
class LandscapeSearch extends Controller
{
    /**
     * Landscape Search Index Page
     *
     * @return void
     */
    ...
    public function search(Request $request)
    {
        ...
        $trademarks = DB::select('select id,name,nice_classes from trademarks use index(indexes)');
            ...
            foreach ($fragments as $fragment) {
                ...
                */
                if (preg_match("/\b\w*$flat\w*\b/iu", $string, $matches)) {
                    ...
                    try {
                        if ($exclusionKeywords != null && in_array($matchedString, $exclusionKeywords)) {
                            break;
                        }
                    } catch (\Exception $e) {
                    ...
                    // Fragment length : 3
                    // else if ($charCount == 3) {
                    else if ($charCount == 3 && $totalConversionScore <= 1.50) {
                        if ($matchCounter <= 1) {
                            $matchedFragments[$key] = $fragment;
                            $counter++;
                        }
                ...
                if (preg_match("/\b\w*$reverse\w*\b/iu", $string, $reverseMatches)) {
                    ...
                    if ($exclusionKeywords != null && in_array($matchedReverseString, $exclusionKeywords)) {
                        break;
                    }
                    ...
                    else if ($reverseCharCount == 3 && $rTotalConversionScore <= 1.50) {
                        if ($reverseMatchCounter == 0) {
                            $matchedFragments[$key] = $fragment;
                            $reverseCounter++;
                        }
                ...
                if ($ts->matchingWordCountRules($wordCount, $reverseCounter) || $ts->matchingWordCountRules($wordCount, $counter) /* || $regexWithSpacesCounter */) {
                    // Log::channel('monitor')->info("Word count: " . $wordCount . " - Counter: " . $counter . " - Reverse Counter: " . $reverseCounter);
        ...
        auth()->user()->decrement('remaining_landscape_search_limit');

        $bulletins = Media::where('is_official', 1)->where('is_saved', 1)->orderBy('bulletin_no', 'desc')->get();
        return view('dashboard.pages.landscape-search')
            ->with('filteredTrademarks', $filteredTrademarks)
            ->with('number_of_results', count($filteredTrademarks))
            ->with('SearchedBulletin', $bulletins)
    ...
    {
        $trademark = Trademark::where('id', $request->input('id'))->first();
        if (!$trademark) {
            return response()->json([
                'status' => 'error',
                'message' => 'No trademark found.'
            ]);
        }

        $trademark->image_reel_path = url('storage/bulletin/' . preg_replace('/\/([^\/]*)$/', '_$1', $trademark->image_path));
        return response()->json([
            'status' => 'success',
            'data' => $trademark
        ]);
